...
class PlugFlowReactorPSRChain:
    ...
    def run(self) -> None:
        ...
        for i in range(self.n):
            ...
            # --- NEU: vollständigen Outlet-Zustand sichern
            self._species_all = list(gas_in.species_names)
            self._X_full_out = gas_in.X.copy()
    ...
    def outlet_state(self) -> Dict[str, object]:
        """
        Vollständiger Outlet-Zustand am Reaktorende:
        { "T": float, "P": float, "X": {species: mole_fraction} }
        """
        if self._species_all is None or self._X_full_out is None:
            raise RuntimeError("PFR wurde noch nicht gerechnet. Rufe .run() zuerst auf.")
        X_dict = {sp: float(xi) for sp, xi in zip(self._species_all, self._X_full_out)}
        return {
            "T": float(self._T[-1]),
            "P": float(self._P[-1]),
            "X": X_dict
...
class CSTR:
    ...
    def __init__(
        ...
        *,
        ...
        # --- NEU: Platzhalter für vollständigen Outlet-State (alle Spezies)
        self._species_all = None  # type: Optional[List[str]]
        self._X_full_out = None  # type: Optional[np.ndarray]
    # -------------------------- Steady State --------------------------
    ...
    def outlet_state(self) -> Dict[str, object]:
        """
        Vollständiger Outlet-Zustand des CSTR:
        { "T": float, "P": float, "X": {species: mole_fraction} }
        Nutzt den letzten Thermo-Zustand des Reaktors (_g_last), damit wirklich ALLE Spezies enthalten sind.
        """
        self._assert_ran()
        if self._g_last is None:
            raise RuntimeError(
                "Kein letzter Reaktorzustand vorhanden. run_steady() oder run_transient() zuerst ausführen.")
        g = self._g_last  # Cantera ThermoPhase des Reaktors
        X_dict = {sp: float(xi) for sp, xi in zip(g.species_names, g.X)}
        return {"T": float(self.T), "P": float(self.P), "X": X_dict}
